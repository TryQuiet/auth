...
export const append = <A extends Action, C>({
  ...
  // the "sender" of this encrypted link is the signer authoring the link
  const { publicKey: senderPublicKey, secretKey: senderSecretKey } = signer.keys.encryption
...
type Params<A extends Action, C> = {
  ...
  /** The identity authoring this link, along with the keys used to encrypt and sign it. */
  signer: Signer
